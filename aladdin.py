...
def PlayRadio(station):
    ...
    if 'pls' in stationlist[station] or 'asx' in stationlist[station]:
        Play_station = "mplayer -ao oss -slave -quiet -cache 256 " + prompt + " -playlist " + stationlist[station].rstrip("\n") + " < /dev/null &"
    else:
        Play_station = "mplayer -ao oss -slave -quiet -cache 256 " + prompt + " " + stationlist[station].rstrip("\n") + " < /dev/null &"
    print Play_station
